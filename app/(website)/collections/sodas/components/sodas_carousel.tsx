...
interface SodasCarouselProps {
  reviews: ProductDetails["reviews"];
}

export function SodasCarousel({ reviews }: SodasCarouselProps) {
  const plugin = useRef(
    AutoScroll({
      speed: 1,
      startDelay: 0,
        ...
        >
          <CarouselContent className="-ml-4 p-5">
            {[...reviews, ...reviews].map((review, i) => (
              ...
              >
                <Card className=" backdrop-blur-sm rounded-xl p-4 h-full flex flex-col gap-6 shadow-[8px_8px_0_0_rgba(117,211,255,0.9)]">
                  {/* Stars */}
